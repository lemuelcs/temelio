...
import { PrismaClient, NivelMotorista } from '@prisma/client';
...
export async function getDashboardMotorista(req: Request, res: Response) {
  try {
    const userId = req.user!.id; // ✅ Usar o ID do usuário do token
    ...
    const niveisConfig: Record<NivelMotorista, NivelConfig> = {
      INICIANTE: { min: 0, max: 49, proximo: 'BRONZE' as NivelMotorista, pontosProximo: 50 },
      BRONZE: { min: 50, max: 69, proximo: 'PRATA' as NivelMotorista, pontosProximo: 70 },
      PRATA: { min: 70, max: 84, proximo: 'OURO' as NivelMotorista, pontosProximo: 85 },
      OURO: { min: 85, max: 94, proximo: 'ELITE' as NivelMotorista, pontosProximo: 95 },
      ELITE: { min: 95, max: 100, proximo: 'ELITE' as NivelMotorista, pontosProximo: 100 }
    };

    const nivelAtual = motorista.nivel || 'INICIANTE';
    ...
    // ✅ CORRIGIDO: Usar rotaAtribuicao (nome correto do model)
    // Rotas concluídas mês atual
    const rotasMesAtual = await prisma.rotaAtribuicao.findMany({
      where: {
        motoristaId,
        status: 'CONCLUIDA',
        dataRota: {
    ...
    const rotasMesAnterior = await prisma.rotaAtribuicao.findMany({
      where: {
        motoristaId,
        status: 'CONCLUIDA',
        dataRota: {
    ...
    // ✅ CORRIGIDO: Adicionar tipos explícitos para evitar erro TS7006
    const faturamentoMesAtual = rotasMesAtual.reduce(
      (sum: number, rota) => sum + Number(rota.valorRota || 0),
      0
    );
    ...
    const rotasPendentes = await prisma.rotaAtribuicao.findMany({
      where: {
        motoristaId,
        status: 'PENDENTE',
        dataRota: {
    ...
    const totalRotasAtribuidas = await prisma.rotaAtribuicao.count({
      where: {
        motoristaId,
        status: {
    ...
    const totalRotasConcluidas = await prisma.rotaAtribuicao.count({
      where: {
        motoristaId,
        status: 'CONCLUIDA'
      }
